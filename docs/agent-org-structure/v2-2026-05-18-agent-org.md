...
Phase or stage information is intentionally excluded from this layer to avoid stale-state drift. Volatile per-project status belongs in the project's own channel pinned messages (e.g., `#fairy` pinned) or repo docs (e.g., `docs/product/decisions/index.md`), not in the agent's deployed prompt.
