...
Phase or stage information is intentionally excluded from this layer to avoid stale-state drift. Volatile per-project status belongs in the project's own channel pinned messages (e.g., the `#{project}` channel's pinned message) or its repo docs (e.g., a project decisions log), not in the agent's deployed prompt.
