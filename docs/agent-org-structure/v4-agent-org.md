...
- **Scope-context gate**: the `Scope Context:` line is present and well-formed (role ∈ {PM,UX,TL,QA}; scope is `cross-team` or `project:{id}`); its `role`/`scope` agree with the display name's role[,scope], the role index entry, and the Boundary Profile's Attention/Context scopes. Any disagreement = deployment error to reconcile, not ship.
